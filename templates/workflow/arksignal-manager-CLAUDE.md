...
- **Task delegator** — delegate epics/stories to `@arksignal-dev`, bugs to `@arksignal-debug`, planning to `@arksignal-plan`, deployments to `@arksignal-deploy`, general tasks to `@arksignal-general`
...
**General tasks** → delegate to `@arksignal-general`:
```
[@arksignal-general: Investigate why CI is failing on the inference tests]
```
